...
type resolutionFinishInput struct {
	ID        int64
...
func (s *Store) FinishResolutionRequest(id int64, status model.ResolutionStatus, errCode, errMsg, graphJSON string) error {
	return s.finishResolutionRequest(resolutionFinishInput{
		ID:        id,
		Status:    status,
		ErrorCode: errCode,
		ErrorMsg:  errMsg,
...
// FinalizeResolutionRequest 记录请求清理阶段的终结结果。
func (s *Store) FinalizeResolutionRequest(id int64, errCode, errMsg string) error {
	return s.finishResolutionRequest(resolutionFinishInput{
		ID:        id,
		Status:    model.ResFailed,
		ErrorCode: errCode,
		ErrorMsg:  errMsg,
		GraphJSON: "",
	})
...
func (s *Store) finishResolutionRequest(in resolutionFinishInput) error {
	finishedAt := nowTime()
	_, err := s.db.Exec(
