...
// ResolveContext 使用给定 context 解析依赖清单。
func (e *Engine) ResolveContext(ctx context.Context, manifest []ManifestItem) Result {
	...
	resolveArtifact = func(name, cstr string, depth int, parent string) {
		...
		art, err := e.artifactByName(resolveCtx, name)
		if err != nil {
			diags = append(diags, Diagnostic{
				Type:    "MISSING",
				Message: fmt.Sprintf("artifact %q not found: %v", name, err),
		...
		candidates, err := e.candidateVersions(resolveCtx, art, isExplicit)
		if err != nil {
		...
		inStack[name] = true
		deps, err := e.dependenciesFor(resolveCtx, chosen.ID)
		if err != nil {
			diags = append(diags, Diagnostic{
				Type:    "MISSING",
				Message: fmt.Sprintf("failed to list dependencies for %q: %v", name, err),
				Details: name,
			})
			inStack[name] = false
...
func (e *Engine) candidateVersions(ctx context.Context, art model.Artifact, explicit bool) ([]model.Version, error) {
	if explicit {
		if e.cat.AllPublishedVersionsContext != nil {
			return e.cat.AllPublishedVersionsContext(ctx, art.ID)
		}
		return e.cat.AllPublishedVersions(art.ID)
	}
	if e.cat.PublishedVersionsContext != nil {
